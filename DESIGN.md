# DESIGN.md — 「青柠胶囊 Mint Capsule」UI 风格 Spec v1.2

> 个人主页设计系统 · 李瑞祥 · AI Agent 产品研发
> 关键词：白色为主 · 薄荷层次带 · 流动的绿 · 胶囊元素 · 卡片信息分组

## 1. 设计原则

1. **白色为主，薄荷做层次**：页面画布纯白 `#FFFFFF`；经历与荣誉两个区块铺极淡薄荷带（`#F5F9F7` 渐变），纵向形成"白-薄荷-白"的节奏，避免全白失焦。
2. **流动的绿**：绿色必须"活"——Hero 三团薄荷极光缓慢漂移呼吸（26s/32s/38s 交错）、时间线轨道有流光下行（5.5s 循环）、页面顶部有随滚动生长的绿色进度条（CSS scroll-driven）、主按钮 hover 时渐变流动。全部只动 `transform`/`opacity`/`background-position`，`prefers-reduced-motion` 下全部静止。
3. **绿色点睛预算**：静态绿色仅允许出现在——主 CTA 按钮、「求职中」状态胶囊、时间线圆点与流光、卡片小图标底、统计数字、hover/focus 反馈；其余一律中性灰绿。
4. **胶囊即语言**：按钮、标签、导航、徽章全部使用 `border-radius: 999px` 胶囊形态。
...
Hero 极光由三团径向渐变（`.hero-glow` 的 before/after/blob）组成，模糊 48px，16%–30% 透明度，缓慢漂移；这是"流动的绿"的主舞台。
